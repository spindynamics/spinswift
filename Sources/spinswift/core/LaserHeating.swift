...
class LaserExcitation: Codable {
  ...
  struct Temperatures: Codable {
    ...
    static func + (a: Temperatures, b: Temperatures) -> Temperatures {
      return Temperatures(
        Electron: (a.Electron + b.Electron), Phonon: (a.Phonon + b.Phonon), Spin: (a.Spin + b.Spin))
    }
  ...
  struct Pulse: Codable {
    ...
    init(
      Form: String? = "Gaussian", Fluence: Double? = Double(), Duration: Double? = Double(),
      Delay: Double? = Double()
    ) {
      self.Form = Form!
  ...
  struct TTM: Codable {
    ...
    struct Coupling: Codable {
      ...
      init(
        ElectronPhonon: Double? = Double(), ElectronSpin: Double? = Double(),
        PhononSpin: Double? = Double()
      ) {
    ...
    init(
      EffectiveThickness: Double? = Double(), InitialTemperature: Double? = Double(),
      Damping: Double? = Double(), HeatCapacity: HeatCapacity? = HeatCapacity(),
  ...
  init(
    CurrentTime: Double? = Double(), temperatures: Temperatures? = Temperatures(),
    pulse: Pulse? = Pulse(), ttm: TTM? = TTM()
  ) {
  ...
  internal func ComputeInstantPower(time: Double) -> Double {
  ...
  internal func computeCp(T: Double, TDebye: Double) -> Double {

    let u: Double = TDebye / T

    // Define the integrand function: f(x) = (x^4 * exp(x)) / (exp(x) - 1)^2
    let integrand: (Double) -> Double = { x in
      return (pow(x, 4) * exp(x)) / pow((exp(x) - 1), 2)
    }

    // Use Gauss-Legendre integration over [0, u]
    ...
    // Apply final scaling: = 4.5 * u^(-2)
    return integral * 4.5 * pow(u, -2)
  }
  ...
  private func LHS2TMvP(time: Double, temperatures: Temperatures) -> Temperatures {
    ...
    let Kb: Double = k_B.value / Joule.value
  ...
  private func LHS3TM(time: Double, temperatures: Temperatures) -> Temperatures {
    ...
    let gps: Double = self.ttm.Coupling!.PhononSpin
    let γ: Double = self.ttm.HeatCapacity!.Electron  // Ce=gamma*Te
    let Cp: Double = self.ttm.HeatCapacity!.Phonon
    ...
    f0 -= (ges / γ) * (1.0 - (Ts / Te))
    rate.Electron = f0 - (1.0 / (τ_ls)) * (Te - T_ref)  // Newton cooling
    rate.Phonon = (gel / Cp) * (Te - Ti) + (gps / Cp) * (Ts - Ti)  // f[1]=dTi/dt
    rate.Spin = (ges / (Cs * pow(Ts, 1.5))) * (Te - Ts) + (gps / (Cs * pow(Ts, 1.5))) * (Ti - Ts)
    return rate
  }

  internal func AdvanceTemperaturesGaussian(method: String, Δt: Double) {
    switch method.lowercased() {
    case "euler", "rk1":
      self.temperatures += Δt * LHS(time: self.CurrentTime, temperatures: self.temperatures)
    case "rk2":
      let k1: Temperatures =
        self.temperatures + 0.5 * Δt * LHS(time: self.CurrentTime, temperatures: self.temperatures)
    ...
    case "rk4":
      ...
      let k2: Temperatures = LHS(
        time: self.CurrentTime + 0.5 * Δt, temperatures: self.temperatures + 0.5 * Δt * k1)
      let k3: Temperatures = LHS(
        time: self.CurrentTime + 0.5 * Δt, temperatures: self.temperatures + 0.5 * Δt * k2)
      let k4: Temperatures = LHS(
        time: self.CurrentTime + Δt, temperatures: self.temperatures + Δt * k3)
